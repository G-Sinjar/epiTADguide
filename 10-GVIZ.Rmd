...
## 1a- filtering annotation table to get cldn10-AB CPGs infos:
```{r}
library(dplyr)
#region_of_interest <- [95433599,95579759]

cpgs_AB <- annotated_with_betas %>%   
  filter(chr == "chr13" & pos >= 95433599 & pos <= 95579759) %>%
  select(CpGs, chr, pos, strand, Islands_Name, Relation_to_Island, UCSC_RefGene_Group, 
         UCSC_RefGene_Name, UCSC_RefGene_Accession, GencodeV41_Group, GencodeV41_Name, 
         GencodeV41_Accession, tail(everything(), 8))

# renaming the sample columns
colnames(cpgs_AB)[(ncol(cpgs_AB)-7):ncol(cpgs_AB)] <- c("unguided_1", "guided_1", "unguided_2", "guided_2", 
                                                         "unguided_3", "guided_3", "unguided_4", "guided_4")
...
# save table to create the granges
cpgs_AB_rownames <- cbind(CpGs = rownames(cpgs_AB), cpgs_AB)
writexl::write_xlsx(cpgs_AB_rownames, "./Output/10-cpgs-AB_for_Granges.xlsx", col_names = TRUE)
```
...
gen <- "hg38"
chr <- as.character(unique(seqnames(gr_cpgs_AB)))
...
fill_colors <- c("CLDN10 Iso-A" = "blue", "Targeted CpG" = "red", "CLDN10 Iso-B" = "green")

# Create AnnotationTrack with CpGs
atrack <- AnnotationTrack(gr_cpgs_AB, 
                          name = "CpGs", 
                          col = NA,       # Border color
...
# Create the plot
plotTracks(list(itrack, gtrack, atrack), groupAnnotation = "id")
```
...
gr_Dmrs <- GRanges(
  seqnames = DMRs$chr,  # Corrected
  ranges = IRanges(start = DMRs$start, end = DMRs$end)  # Removed unnecessary width
)
...
### create track
```{R}
DmrTrack <- AnnotationTrack(gr_Dmrs, name = "DMRs", col = "pink", fill = "pink",feature = paste("P.val =", round(gr_Dmrs$p.value, 5)))
...
plotTracks(list(itrack, gtrack, atrack,dTrack, TadTrack,DmrTrack), groups = rep(c("unguided", "guided"), each = 4), type = c("a", "p") , from = 95433599, to = 95579759)
...
# as a PDF file
pdf("./Plots/GVIZ_CLDN10_AB_5.pdf", width = 20, height = 11)  # dimensions in inches
plotTracks(list(itrack, gtrack, atrack,dTrack, TadTrack,DmrTrack), groups = rep(c("unguided", "guided"), each = 4), type = c("a", "p") , groupAnnotation = "id",from = 95433599, to = 95579759)
