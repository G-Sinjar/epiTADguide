...
# 2. Change the reference level to "unguided"
if (!is.factor(pheno)) {
  pheno <- factor(pheno)
}
...
# DMRs with cutoff = 0.15, B=100
```{r}
library(doParallel)
registerDoParallel(cores = 6)

dmrs_0.01_smooth <- bumphunter(ratio_geno_Swan_0.01_NoSNP, design = designMatrix, cutoff = 0.15, B=100, type="Beta", maxGap = 500, smooth = TRUE) 
# 16 DMRs 
```
...
```{r}
DMRtable <- dmrs_0.01_smooth$table
...
#------ reorder the columns --------------
column_order <- c("DMR_Nr", "chr","start", "end", "length", setdiff(names(DMRtable), c("DMR_Nr","chr" ,"start", "end", "length")))
...
# 2 Add columns (pval, beta and M differnces) to the cpgs_DMR1 
```{r}
...
## create a table for boxplot of cpgs in one DMR1  
```{r}
...
# 2 creating a long format table ready for boxplots of all cpgs in one DMR
```{r}
library(tidyr)
