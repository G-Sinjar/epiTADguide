...
chrom = "chr2"
resolution_kb = 25
path_mcool_file = "../PythonProject/test/4DNFIIH3SM5N"
...
#path_chr_res_txtfile
input_matrix <- "../PythonProject/test/TADs_CAKI2/contact_matrix/CAKI2_chr13_25kb_contact_matrix.txt"
jar_path <- "../PythonProject/deDoc2-main/deDoc2.jar"
binsize_kb <- as.integer(resolution_bp)/1000           
java_memory <- "16g"      
```


# 1- processing mcool files
```{r}
#Do once
# Create a virtualenv (or conda env) named "shiny_py"
#reticulate::virtualenv_create("shiny_py")

# Install cooler and other Python packages
#reticulate::py_install(c("cooler", "numpy"), envname = "shiny_py")
...
result <- process_contact_matrix(
  ...
  path_mcool_file = path_mcool_file
)

# Print result like in Python
if (result$success) {
  cat(result$message, "\n")
  cat("Total contacts in the whole map:", format(result$total_contacts, big.mark = ","), "\n")
} else {
  cat(result$message, "\n")
...
# notes: the pathes should be from python input
# add the file name from the input of the previous py script and get the normalizePath() without the final output file name 
#tell the user what kein of matrix/file format it shouold be

# ---- CHECKS ----
if (!requireNamespace("processx", quietly = TRUE)) {
  stop("❌ The 'processx' package is required. Please install it with install.packages('processx')")
}

if (Sys.which("java") == "") {
  stop("❌ Java is not found in your system PATH.")
}

if (!file.exists(jar_path)) stop(paste("❌ .jar file not found:", jar_path))
if (!file.exists(input_matrix)) stop(paste("❌ Input matrix not found:", input_matrix))

# ---- CREATE OUTPUT PATH ----
input_dir <- dirname(normalizePath(input_matrix))
output_dir <- file.path(input_dir, "TADcaller_Results")

if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

output_base <- file.path(output_dir, paste0(tissue, "_", chrom, "_", binsize_kb, "kb_TADlines"))
...
cat("🚀 Running deDoc2 Java TADcaller...\n")
# ---- Run Java Process ----
result <- tryCatch({
...
if (result$status == 0) {
  cat(result$stdout)
  cat("✅ TADcaller deDoc2 ran successfully.\n")
  cat(paste0("📁 Results saved in: ", normalizePath(output_base, mustWork = FALSE), ".TAD and .window.TAD\n"))
} else {
  cat("❌ Error running TADcaller deDoc2:\n")
  
  # Handle common error
  if (grepl("Unable to access jarfile", result$stderr)) {
    cat("📛 Unable to access the .jar file.\n")
    cat("📌 Please double-check that the path is correct and the file exists:\n")
    cat(paste0("    ➤ ", normalizePath(jar_path, mustWork = FALSE), "\n"))
    cat("💡 Tip: You can drag and drop the .jar file into the app or file dialog to avoid typos.\n")
  } else {
    # Generic fallback
    cat(result$stderr, "\n")
  }
