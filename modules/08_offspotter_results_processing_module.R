...
# like Off-Spotter or similar. It reads multiple tab-separated files,
    ...
    div(
      ...
      layout_columns(
        ...
        div(
          ...
          helpText("Each Off-target ID follows the format: G1.M3.chr19.39304687, where G1 = Guide 1, M3 = 3 mismatches, chr19 = chromosome, and 39304687 = start position."),
    ...
    observeEvent(input$run_processing, {
      ...
      withProgress(message = "Processing off-target files...", value = 0, {
        for (i in seq_along(file_names)) {
          ...
          msgs <- append(msgs, paste0("✅ Step 1 - '", file_name, "' read successfully"))
          
          incProgress(i / (length(file_names) * 4) + 0.1, detail = paste("Verifying columns for", file_name, "..."))
          if (ncol(guide_data) != length(col_names)) {
            msgs <- append(msgs, paste0("❌ Step 2 - Column mismatch for '", file_name, "': expected ", length(col_names), ", got ", ncol(guide_data)))
            next
          }
          msgs <- append(msgs, paste0("✅ Step 2 - '", file_name, "' column count verified"))
          ...
          processed <- tryCatch({
            ...
            msgs <<- append(msgs, paste0("❌ Step 3 - Processing '", file_name, "' failed: ", e$message))
            return(NULL)
          })
          if (is.null(processed)) next
          msgs <- append(msgs, paste0("✅ Step 3 - '", file_name, "' data processed"))
          
          all_guides[[file_name]] <- processed
          msgs <- append(msgs, paste0("✅ Step 4 - '", file_name, "' data stored"))
        }
        ...
        if (nrow(final) == 0) {
          msgs <- append(msgs, "⚠️ Warning: After filtering, no off-targets (mismatches > 0) were found.")
        ...
        msgs <- append(msgs, "✅ Step 5b - Filtered out on-targets and generated IDs")
        ...
        if (!is.null(project_output_dir())) { 
          ...
          tryCatch({
            ...
            msgs <- append(msgs, paste0("💾 Step 6 - Saved processed data as csv at: ", output_path))
          }, error = function(e) {
            msgs <<- append(msgs, paste0("❌ Step 6 - Saving csv failed: ", e$message))
          })
        } else {
          msgs <- append(msgs, "ℹ️ Step 6 - Skipping saving data: Project output directory not provided. To save the table click on the download button")
        }
