...
atrack <- AnnotationTrack(gr_cpgs, chromosome = chr, name = "CpGs", 
                          col = NA,       # Border color
...
# Create the plot
plotTracks(list(itrack, gtrack, atrack), from = from, to = to, showBandId = TRUE,cex.bands = 0.5, "Other"= "lightblue", "CLDN10_IsoA" = "blue", "Targeted_CpG" = "red", "CLDN10_IsoB" = "green",stacking = "dense")
```
...
dTrack <- DataTrack(gr_cpgs, chromosome = chr,
                    name = "CpGs Beta values",
                    data = last_8_data)
 
plotTracks(dTrack, groups = rep(c("unguided", "guided"), each = 4),
           type = c("a", "p"), from = from , to = to, aggregateGroups = TRUE, aggregation = "mean")
```
...
plotTracks(TadTrack,groupAnnotation = "feature", from = from, to = to)
```
...
### 1H.4 Get transcript ranges (as GRanges)
```{r}
...
displayPars(TadTrack) <- list(groupAnnotation = "feature") 
...
plotTracks(
  list(itrack, gtrack, atrack, DmrTrack, cpgIslandTrack, dTrack,geneTrack,ensTransTrack, TadTrack),
  from = from,
  to = to,
  showBandId = TRUE,
  cex.bands = 0.7,
...
# as a PDF file
pdf("./Plots/GVIZ_CLDN10_AB_12.pdf", width = 27, height = 15)  # dimensions in inches
displayPars(DmrTrack) <- list(groupAnnotation = "feature")  
displayPars(cpgIslandTrack) <- list(groupAnnotation = "feature") 
displayPars(TadTrack) <- list(groupAnnotation = "feature") 
displayPars(atrack) <- list(stacking= "dense") 
