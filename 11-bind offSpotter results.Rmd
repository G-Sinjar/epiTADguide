...
read_guide_files <- function(file_path, num_guides) {
  ...
  for (i in 1:num_guides) {
    ...
    # Read each guide file
    guide_data <- readr::read_delim(guide_file, delim = "\t", col_names = FALSE, show_col_types = FALSE)
  ...
  combined_guides <- dplyr::bind_rows(all_guides)
  
  # !!! USE dplyr::filter !!!
  just_offtargets <- combined_guides %>%
    dplyr::filter(`Number_of_mismatches` != 0)
  
  return(just_offtargets)
}
```
...
num_guides <- 6
combined_Offspotter_guideResults <- read_guide_files(offspoter_result_path, num_guides)



View(combined_Offspotter_guideResults)
```
...
gr_offtargets <- GRanges(
  ...
  #strand = combined_Offspotter_guideResults$Strand,
  # Add all other columns as metadata
  mcols = combined_Offspotter_guideResults[, !(colnames(combined_Offspotter_guideResults) %in% c("Chrom", "Strand", "Start_minus_70", "End_plus_70"))]
)
